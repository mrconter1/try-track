...
def find_quads(points):
    ...
    if len(valid_quads) < 3:
        # Not enough quads to be confident, return empty
        return []
    ...
    # Filter: Keep quads within 0.5x to 1.5x of median area
    area_filtered_quads = []
    ...
    if len(valid_vp1) < 3 or len(valid_vp2) < 3:
    ...
    # Threshold: 1.5x median distance
    thresh1 = max(median_dist1 * 1.5, 1000)  # Min 1000px to avoid overly strict
    thresh2 = max(median_dist2 * 1.5, 500)
    ...
    # Final check: need at least 3 quads
    if len(vp_filtered_quads) < 3:
        return []
