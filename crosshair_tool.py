...
class App:
    def __init__(self, root, args, initial_frame, video_path, total_frames):
        ...
        ttk.Label(control_frame, text="CLAHE:").grid(row=3, column=0, sticky=tk.W, pady=2)
        self.clahe_checkbox = ttk.Checkbutton(control_frame, variable=self.clahe_enabled, command=self.update_image)
        self.clahe_checkbox.grid(row=3, column=1, sticky=tk.W)
        
        # CLAHE Clip Limit
        ttk.Label(control_frame, text="Clip:").grid(row=3, column=2, sticky=tk.W, padx=(10, 0))
        ...
        self.clahe_clip_slider.grid(row=3, column=3, sticky="ew")
        self.clahe_clip_label = ttk.Label(control_frame, text=f"{self.clahe_clip_limit.get():.1f}", width=5)
        self.clahe_clip_label.grid(row=3, column=4, padx=5)
    ...
    def update_image(self, *args):
        ...
        # Draw crosshair: two perpendicular lines (horizontal and vertical)
